...
import {STUDENT_STATUS_LABELS, STUDENT_STATUS_TONES} from "@/features/students/constants.ts";
...
const toDisplayName = (student: StudentProfile | null): string => {
    if (!student) {
...
const StudentEditPage = ({readOnly = false}: StudentEditProps) => {
    ...
    useEffect(() => {
        ...
        const fetchStudent = async () => {
            ...
            setIsLoading(true);
            try {
                const {data} = await axios.get<{data: StudentProfile}>(`/api/v1/students/${id}`);
                if (!isMounted) {
                    return;
                }
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const {data} = await axios.patch<{data: StudentProfile}>(`/api/v1/students/${student.id}`, payload);
            setStudent(data.data);
    ...
    const summary = useMemo(() => {
        ...
        return {
            ...
            statusValue,
            statusLabel: STUDENT_STATUS_LABELS[statusValue] ?? capitalize(student?.status),
                    ...
                    <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div className="rounded-xl border border-gray-200 bg-white p-4">
                            ...
                            <Badge
                                ...
                                variant={STUDENT_STATUS_TONES[summary.statusValue] ?? "outline"}
                            >
                                {summary.statusLabel}
                            </Badge>
                        </div>
